...
def test_scan_help_lists_all_documented_flags(runner):
    result = runner.invoke(cli, ["scan", "--help"])
    for flag in ["--static-only", "--offline", "--deep", "--token", "--fail-on", "--output", "--batch"]:
...
def test_end_to_end_secret_server_flags_high(runner, server_with_secret):
    result = runner.invoke(cli, ["scan", str(server_with_secret), "--offline", "--fail-on", "high"])
    assert result.exit_code == 1, f"Expected non-zero exit on --fail-on high. Output:\n{result.output}"


def test_end_to_end_json_output_is_valid(runner, server_with_secret):
    import json
    result = runner.invoke(cli, ["scan", str(server_with_secret), "--offline", "--output", "json"])
...
def test_end_to_end_sarif_output_is_valid(runner, server_with_secret):
    import json
    result = runner.invoke(cli, ["scan", str(server_with_secret), "--offline", "--output", "sarif", "--static-only"])
...
def test_static_only_runs_and_passes_clean(runner, clean_server_path):
    result = runner.invoke(cli, ["scan", str(clean_server_path), "--offline", "--static-only"])
    assert result.exit_code == 0
    assert "PASS" in result.output
...
def test_batch_scans_each_listed_path(runner, tmp_path, clean_server_path, server_with_secret):
    listing = tmp_path / "servers.txt"
    listing.write_text(
        f"{clean_server_path}\n"
        f"# a comment line\n"
        f"\n"
        f"{server_with_secret}\n"
    )
...
def test_static_only_skips_llm_even_with_manifest(runner, tmp_path, monkeypatch):
    ...
    project = tmp_path / "with_manifest"
    project.mkdir()
    (project / "mcp.json").write_text('{"tools": [{"name": "fetch", "description": "x"}]}')
...
def test_local_llm_graceful_degradation_when_ollama_unavailable(runner, tmp_path, monkeypatch):
...
def test_config_flag_scans_each_declared_server(runner, tmp_path, clean_server_path, monkeypatch):
    import json as _json
    cfg = tmp_path / "claude_desktop_config.json"
    cfg.write_text(_json.dumps({
        "mcpServers": {
            "local": {"command": "python", "args": [str(clean_server_path / "nothing.py")]},
    ...
    result = runner.invoke(cli, ["scan", str(cfg), "--config", "--offline", "--static-only"])
    assert result.exit_code == 0, f"Output: {result.output}"
    # Both servers should appear as scan targets.
    assert "npm:@modelcontextprotocol/server-github" in result.output
    assert str(clean_server_path) in result.output or clean_server_path.name in result.output
...
def test_unknown_source_prefix_errors(runner):
    result = runner.invoke(cli, ["scan", "docker:foo/bar", "--offline", "--static-only"])
    assert result.exit_code == 2
    assert "docker" in result.output.lower() or "unknown source" in result.output.lower()
...
def test_github_source_dispatched_through_cli(runner, monkeypatch, tmp_path):
    ...
    result = runner.invoke(cli, ["scan", "github:nyuway/scanner", "--offline", "--static-only"])
    assert result.exit_code == 0, f"Output: {result.output}"
    assert called["n"] == 1
...
def test_batch_fail_on_aggregates_across_targets(runner, tmp_path, clean_server_path, server_with_secret):
    listing = tmp_path / "servers.txt"
