...
@pytest.mark.parametrize("rule_name,content", [
    (
...
def test_each_yara_rule_fires(tmp_path, rule_name, content):
    ...
    assert rule_name in rules_fired, (
        f"Expected rule '{rule_name}' to fire on its own sample. "
        f"Got: {rules_fired}"
    )
...
def test_yara_rules_file_itself_does_not_self_trigger(tmp_path):
    ...
    (project / "mcp_threats.yar").write_text(DEFAULT_RULES_FILE.read_text(encoding="utf-8"))
    findings = run_yara(str(project))
