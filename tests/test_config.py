...
from nyuwaymcpscanner.sources.config import (
    parse_config, ConfigParseError, resolvable_specs,
)
...
def test_node_script_resolved(tmp_path):
    cfg = _write(tmp_path, {
        "mcpServers": {
            "custom": {"command": "node", "args": ["./local-server.js"]}
        }
    })
    entries = parse_config(cfg)
...
def test_direct_executable_path(tmp_path):
    cfg = _write(tmp_path, {
        "mcpServers": {
            "bin": {"command": "/opt/mcp/bin/server", "args": []}
        }
    })
    entries = parse_config(cfg)
...
def test_npx_with_only_flags(tmp_path):
    cfg = _write(tmp_path, {
        "mcpServers": {"f": {"command": "npx", "args": ["-y", "--quiet"]}}
    })
    entries = parse_config(cfg)
    assert entries[0].spec is None
